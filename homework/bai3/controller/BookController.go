...
func CheckBookExits(idBook int64) bool {
	isExits := false
	for _, book := range repo.Books.GetAllBooks() {
		if idBook == book.Id {
			return true
		}
	}
...
func FindBookById(idBook int64) *model.Book {
	book, err := repo.Books.FindBookById(int64(idBook))
	if err != nil {
...
func UpdateBookV2(book *model.Book) {
	err := repo.Books.UpdateBook(book)
	if err != nil {
		fmt.Println(err.Error())
	}
}
...
func UpdateBookWhenChangeReview() {
